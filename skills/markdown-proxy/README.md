...
Send any URL to Codex or Claude Code, and it automatically fetches the full content as Markdown. Four special platforms have dedicated extraction:
...
- [ ] Codex or [Claude Code](https://docs.anthropic.com/en/docs/claude-code) installed
...
Verify:
```bash
Get-Item "$HOME\\.codex\\skills\\markdown-proxy\\SKILL.md"
```

PowerShell tip:

- Use `python`, not `python3`
- Use `curl.exe`, not bare `curl`
- Default skill path is `"$HOME\\.codex\\skills\\markdown-proxy"`

### Usage

Just send Codex or Claude Code a URL:
...
给 Codex 或 Claude Code 发一个 URL，自动抓取完整内容并转为 Markdown。支持四种特殊平台的专用抓取：
...
- [ ] 已安装 Codex 或 [Claude Code](https://docs.anthropic.com/en/docs/claude-code)
- [ ] **curl**（macOS/Linux 自带）
...
- 优先使用 `python`，不要假设有 `python3`
- 优先使用 `curl.exe`，避免命中 PowerShell 的 `curl` alias
- 默认技能路径是 `"$HOME\\.codex\\skills\\markdown-proxy"`

### 使用示例

直接给 Codex 或 Claude Code 发 URL：
